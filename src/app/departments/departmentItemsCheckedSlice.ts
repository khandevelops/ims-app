...
export const getDepartmentItem = (params: {
  pathName: string;
  page: number;
  size: number;
}) => {
  return axios.put(
    `${baseUrl}${params.pathName}/list?page=${params.page}&size=${params.size}`
  );
};
...
export const updateDepartmentItem = (params: {
  ...
  return axios.put(
    `${baseUrl}${params.departmentName}/${params.id}`,
    params.departmentItem
  );
};

export const getDepartmentItems = (params: {
  pathName: string;
  page: number;
  size: number;
}) => {
...
export const getDepartmentItemsThunk = createAsyncThunk(
  ...
  async (params: { pathName: string; page: number; size: number }) => {
    const response = await getDepartmentItems({
...
export const selectDepartmentItems = (state: RootState) =>
  state.departmentItemsStore;
