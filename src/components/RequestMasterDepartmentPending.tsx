...
const RequestMasterDepartmentPending = () => {
  const requestMasterItemsPendingSelector = useAppSelector(
    selectRequestMasterItemsPending
  );
  ...
  const handleCheckboxChange = (
    ...
    departmentMasterItem: IRequestMasterItem
  ) => {
    const exists =
      requestMasterItemsPendingCheckedSelector.requestMasterItemsPendingChecked.some(
      ...
      dispatch(
        changeRequestItemsPendingChecked(
          requestMasterItemsPendingCheckedSelector.requestMasterItemsPendingChecked.filter(
            (item) =>
              item.request_item_id !== departmentMasterItem.request_item_id
          )
        )
      );
    }
  ...
  const handleChangeQuantity = (
    ...
    dispatch(
      changeRequestMasterItemsPending({
        ...
        content: requestMasterItemsPendingSelector.response.content.map(
  ...
  const handleUpdateQuantity = (
    event: KeyboardEvent,
    requestMasterItem: IRequestMasterItem
  ) => {
    if (event.key === "Enter") {
      dispatch(
                        ...
                        checked={
                          requestMasterItemsPendingCheckedSelector.requestMasterItemsPendingChecked.find(
                            (item) =>
                              ...
                              requestMasterItem.request_item_id
                          ) !== undefined
                        }
                      />
                    </StyledTableCell>
