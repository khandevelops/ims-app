...
export const requestMasterItemsUpdateSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            .addCase(updateRequestMasterItemsThunk.pending, (state) => { state.status = 'loading' })
            .addCase(updateRequestMasterItemsThunk.fulfilled, (state, action) => { state.response = action.payload })
            .addCase(updateRequestMasterItemsThunk.rejected, (state) => { state.status = 'failed' })
    }
})
