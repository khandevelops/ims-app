...
export const getRequestMasterItemsPending = (state: string, page: number) => {
  return axios.get(
    `${baseUrl}/request-master/${state}/list/transformed/pending?page=${page}`
  );
};
...
export const getRequestMasterItemsPendingThunk = createAsyncThunk(
  ...
  async (params: { state: string; page: number }) => {
    const response = await getRequestMasterItemsPending(
      params.state,
      params.page
    );
...
export const selectRequestMasterItemsPending = (state: RootState) =>
  state.requestMasterItemsPendingStore;

export const { changeRequestMasterItemsPending } =
  requestMasterItemsPendingSlice.actions;
