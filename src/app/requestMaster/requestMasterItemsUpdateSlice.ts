...
export const updateRequestMasterItems = (
  state: string,
  requestItems: IRequestMasterItem[]
) => {
  return axios.patch(
    `${baseUrl}/request-master/${state}/update/items`,
    requestItems
  );
};
...
export const updateRequestMasterItemsThunk = createAsyncThunk(
  ...
  async (params: { state: string; requestItems: IRequestMasterItem[] }) => {
    const response = await updateRequestMasterItems(
      params.state,
      params.requestItems
    );
...
export const selectRequestMasterItemsUpdate = (state: RootState) =>
  state.requestMasterItemsUpdateStore;
