...
const UpdateItemForm = () => {
  ...
  const handleChange = (event: ChangeEvent<HTMLInputElement>) => {
    if (event.target.name === "location") {
      dispatch(
        changeStoreRoomItem({ storeRoomItem: { location: event.target.value } })
      );
    }
    if (event.target.name === "usage_level") {
      dispatch(
        changeStoreRoomItem({
          storeRoomItem: { usage_level: event.target.value },
        })
      );
    }
    if (event.target.name === "minimum_quantity") {
      dispatch(
        changeStoreRoomItem({
          storeRoomItem: { minimum_quantity: event.target.value },
        })
      );
    }
    if (event.target.name === "maximum_quantity") {
      dispatch(
        changeStoreRoomItem({
          storeRoomItem: { maximum_quantity: event.target.value },
        })
      );
    }
  };
  ...
  const handleDateChange = (value: Date | null, columnName: string) => {
    if (columnName === "expiration_date") {
      dispatch(
        changeStoreRoomItem({ storeRoomItem: { expiration_date: value } })
      );
    }
          ...
          <LocalizationProvider dateAdapter={AdapterMoment}>
            <DateTimePicker
              ...
              onChange={(value: Date | null) =>
                handleDateChange(value, "expiration_date")
              }
              renderInput={(params) => (
