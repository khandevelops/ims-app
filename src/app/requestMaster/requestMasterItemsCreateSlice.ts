...
export const createRequestMasterItems = (
  state: string,
  requestMasterItems: IRequestMasterItem[]
) => {
  return axios.post(
    `${baseUrl}/request-master/${state}/create`,
    requestMasterItems
  );
};
...
export const createRequestMasterItemsThunk = createAsyncThunk(
  ...
  async (params: {
    ...
    const response = await createRequestMasterItems(
      params.state,
      params.requestMasterItems
    );
...
export const selectRequestMasterItemsCreate = (state: RootState) =>
  state.requestMasterItemsCreateStore;
