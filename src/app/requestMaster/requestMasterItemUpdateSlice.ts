...
export const updateRequestMasterItem = (
  state: string,
...
export const updateRequestMasterItemThunk = createAsyncThunk(
  ...
  async (params: { state: string; requestMasterItem: IRequestMasterItem }) => {
    const response = await updateRequestMasterItem(
      params.state,
...
export const selectRequestMasterItemUpdate = (state: RootState) =>
  state.requestMasterItemUpdateStore;
